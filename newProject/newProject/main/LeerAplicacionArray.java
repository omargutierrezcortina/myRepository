...
public class LeerAplicacionArray {
	...
	public static void main(String[] args) {
		...
		case 1:
			...
			AplicacionWeb[] aplicacionesWeb = { aplicacionWebService.crearAplicacionWeb(),
					aplicacionWebService.crearAplicacionWeb() };

			// Para imprimir la información de las aplicaciones web
			for (int i = 0; i < aplicacionesWeb.length; i++) {
				System.out.println("  AppWeb " + (i + 1) + ": " + aplicacionesWeb[i]);
			}
			break;
		case 2:
			...
			AplicacionMovil[] aplicacionesMovil = { aplicacionMovilService.crearAplicacionMovil(),
					aplicacionMovilService.crearAplicacionMovil() };

			// Para imprimir la información de las aplicaciones móvil
			for (int i = 0; i < aplicacionesMovil.length; i++) {
				System.out.println("  AppMovil " + (i + 1) + ": " + aplicacionesMovil[i]);
			}
			break;
		case 3:
			...
			AplicacionIOT[] aplicacionesIOT = { aplicacionIOTService.crearAplicacionIOT(),
					aplicacionIOTService.crearAplicacionIOT() };

			// Para imprimir la información de las aplicaciones IOT
			for (int i = 0; i < aplicacionesIOT.length; i++) {
				System.out.println("  AppIOT " + (i + 1) + ": " + aplicacionesIOT[i]);
			}
			break;
		default:
